...
function events() {
  ref.saveBtn.addEventListener("click", async () => {
    ...
    if (name && password && mailOrPhone) {
      const res = await axios.post(`${api}/passwords`, {
        name: name.trim(),
        password,
        mailOrPhone,
        additional,
  ...
  ref.searchInput.addEventListener("input", (e) => {
    ...
    if (!val) {
      ...
      passwordArrSearch = passwordArr.filter((el) =>
...
function updatePassword() {
  ...
  for (const { name, mailOrPhone } of passwordArrSearch) {
    ...
    elem
      .querySelector(".view-additional")
      .addEventListener("click", async () => {
        if (
          elem.querySelector(".password-addition span").getAttribute("show") ===
          "false"
        ) {
          const res = await axios.get(`${api}/passwords/additional/${name}`);
          elem
            .querySelector(".password-addition span")
            .setAttribute("show", "true");
          elem.querySelector(".password-addition span").textContent = res.data
            .additional
            ? res.data.additional
            : "-";
        } else {
