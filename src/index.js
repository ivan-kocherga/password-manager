...
function initServe() {
  ...
  serve.get("/passwords", (req, res) => {
    fs.readFile(
      path.join(__dirname, "passwords.encrypt.json"),
      "utf8",
      (err, data) => {
        if (err) {
          return;
  ...
  serve.get("/passwords/decrypt", async (req, res) => {
    ...
    for (const fileElem in file) {
      txt += `Название: ${fileElem}. Телефон или почта: ${
        file[fileElem].mailOrPhone
      }. Пароль: ${decrypt(file[fileElem].password)}. Дополнительно: ${
        file[fileElem].additional
      }.\n`;
    }
  ...
  serve.get("/passwords/:name", async (req, res) => {
    ...
    const elem = Object.entries(file).find(
      ([_name]) => req.params.name.replace("%20", " ") === _name
    )[1];
  ...
  serve.get("/passwords/additional/:name", async (req, res) => {
    ...
    const elem = Object.entries(file).find(
      ([name]) => req.params.name.replace("%20", " ") === name
    )[1];
